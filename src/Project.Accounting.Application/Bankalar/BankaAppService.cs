...
[Authorize(AccountingPermissions.Banka_.Default)]
public class BankaAppService : AccountingAppService, IBankaAppService
{
    ...
    [Authorize(AccountingPermissions.Banka_.Create)]
    public virtual async Task<SelectBankaDto> CreateAsync(CreateBankaDto input)
    {
        ...
        //ui dan gelen CreateBankaDto inputunu Entitymiz olan Banka haline mapleme 
        var entity = ObjectMapper.Map<CreateBankaDto, Banka>(input);
        await _bankaRepository.InsertAsync(entity);//entity burada db de oluşturuluyor

        return ObjectMapper.Map<Banka, SelectBankaDto>(entity);
    }

    [Authorize(AccountingPermissions.Banka_.Update)]
    ...
    [Authorize(AccountingPermissions.Banka_.Delete)]
